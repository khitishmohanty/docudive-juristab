import time
import json
import os
import re

from services.gemini_client import call_gemini_api
from utils.prompt_loader import load_text_prompt
from utils.json_utils import _clean_json_string

gemini_layout_prompt_text = load_text_prompt("gemini_layout_prompt.txt")

def _call_gemini_for_layout(
    pdf_chunk_base64: str,
    start_page_actual: int,
    num_pages_in_chunk: int,
    genai_output_dir: str,
    metrics: dict
) -> dict:
    global gemini_layout_prompt_text
    
    gemini_result_for_return = {}
    gemini_raw_text = ""
    gemini_layout_start_time = time.time()
    
    if num_pages_in_chunk == 1:
        output_filename = f"page_{start_page_actual}_genai.json"
    else:
        end_page_actual = start_page_actual + num_pages_in_chunk - 1
        output_filename = f"page_{start_page_actual}_{end_page_actual}_genai.json"
    output_file_path = os.path.join(genai_output_dir, output_filename)

    try:
        if not gemini_layout_prompt_text or not gemini_layout_prompt_text.strip():
            raise ValueError("Gemini layout prompt text from gemini_layout_prompt.txt is empty or invalid.")
        
        gemini_api_parts = [{"text": gemini_layout_prompt_text}]
        gemini_api_call_response = call_gemini_api(
            image_base64=pdf_chunk_base64,
            prompt_parts=gemini_api_parts,
            mime_type="application/pdf"
        )
        metrics["time_sec_gemini_layout"] = time.time() - gemini_layout_start_time
        gemini_raw_text = gemini_api_call_response.get("text", "")
        
        metrics.update({
            "gemini_api_status": 200,
            "gemini_input_tokens": gemini_api_call_response.get("input_tokens", 0),
            "gemini_output_tokens": gemini_api_call_response.get("output_tokens", 0),
            "gemini_cost_usd": gemini_api_call_response.get("cost", 0.0)
        })

        cleaned_gemini_json_str = _clean_json_string(gemini_raw_text)
        metrics["gemini_response_length"] = len(cleaned_gemini_json_str or "")
        
        parsed_data_for_file_saving = None
        processed_items_list = []

        if cleaned_gemini_json_str:
            try:
                parsed_data = json.loads(cleaned_gemini_json_str)

                if isinstance(parsed_data, list):
                    for node in parsed_data:
                        if isinstance(node, dict):
                            page_offset = node.get("page_index_in_chunk")
                            if page_offset is not None and isinstance(page_offset, int) and 0 <= page_offset < num_pages_in_chunk:
                                node["page_number"] = start_page_actual + page_offset
                            elif num_pages_in_chunk == 1:
                                node["page_number"] = start_page_actual
                            else:
                                node["page_number"] = start_page_actual
                                node["page_number_assignment_warning"] = f"Node lacks valid 'page_index_in_chunk'. Defaulted page_number to {start_page_actual} (first page of chunk)."
                        processed_items_list.append(node)
                    
                    gemini_result_for_return = {
                        "items": processed_items_list,
                        "processed_chunk_page_range": [start_page_actual, start_page_actual + num_pages_in_chunk - 1],
                        "_source_data_type": "list"
                    }
                    parsed_data_for_file_saving = gemini_result_for_return
                    print(f"✅ Parsed Gemini list response for chunk pages {start_page_actual}-{start_page_actual + num_pages_in_chunk - 1}.")

                elif isinstance(parsed_data, dict):
                    
                    def get_page_offset_from_parsed_key(key_str: str, max_offset_exclusive: int) -> int | None:
                        # Tries to match "Page1", "page_1", "pg1", "1", etc.
                        key_lower = key_str.lower()
                        match_text_num = re.fullmatch(r"(?:page|pg)?[_ ]*(\d+)", key_lower)

                        if match_text_num:
                            num_in_key = int(match_text_num.group(1))
                            offset = -1
                            if key_str.isdigit(): # Purely numeric key "0", "1" -> assume 0-based offset
                                offset = num_in_key
                            elif "page" in key_lower or "pg" in key_lower : # Key like "Page1", "pg1" -> assume 1-based index from key
                                offset = num_in_key - 1 
                            
                            if 0 <= offset < max_offset_exclusive:
                                return offset
                        return None

                    page_structured_items_for_file = {} 
                    unassigned_dict_keys_values = {} 
                    found_structured_page_data_in_dict = False

                    for key, value_list in parsed_data.items():
                        page_offset = get_page_offset_from_parsed_key(key, num_pages_in_chunk)
                        if page_offset is not None and isinstance(value_list, list):
                            found_structured_page_data_in_dict = True
                            actual_page_num_for_nodes = start_page_actual + page_offset
                            
                            for node in value_list:
                                if isinstance(node, dict):
                                    node["page_number"] = actual_page_num_for_nodes
                            processed_items_list.extend(value_list)
                            # For the file, use a consistent key like "PageX" where X is actual page number
                            page_structured_items_for_file[f"Page{actual_page_num_for_nodes}"] = value_list
                        else:
                            unassigned_dict_keys_values[key] = value_list

                    if found_structured_page_data_in_dict:
                        gemini_result_for_return = {
                            "items": processed_items_list,
                            "processed_chunk_page_range": [start_page_actual, start_page_actual + num_pages_in_chunk - 1],
                            "_source_data_type": "page_structured_dict_recognized"
                        }
                        # Merge successfully parsed page data with any other unrelated keys
                        parsed_data_for_file_saving = {**page_structured_items_for_file, **unassigned_dict_keys_values}
                        # Remove old error keys if they were in unassigned_dict_keys_values from a previous run's output
                        if "error_ambiguous_multi_page_dict" in parsed_data_for_file_saving:
                            del parsed_data_for_file_saving["error_ambiguous_multi_page_dict"]
                        if "page_number" in parsed_data_for_file_saving and isinstance(parsed_data_for_file_saving["page_number"], int) and len(page_structured_items_for_file) > 0 :
                             # If we successfully parsed pages, the top-level 'page_number' from an old error is misleading.
                            del parsed_data_for_file_saving["page_number"]

                        print(f"✅ Parsed recognized page-structured dict from Gemini for chunk pages {start_page_actual}-{start_page_actual + num_pages_in_chunk - 1}.")
                    
                    else: # Not a recognized page-structured dict; it's a single dictionary.
                        if num_pages_in_chunk == 1:
                            if "page_number" not in parsed_data:
                                parsed_data["page_number"] = start_page_actual
                            items_in_dict = parsed_data.get("items")
                            if isinstance(items_in_dict, list):
                                for node in items_in_dict:
                                    if isinstance(node, dict) and "page_number" not in node:
                                        node["page_number"] = start_page_actual
                                processed_items_list.extend(items_in_dict)
                                gemini_result_for_return = {
                                    "items": processed_items_list,
                                    "processed_chunk_page_range": [start_page_actual, start_page_actual],
                                    "_source_data_type": "single_dict_with_items"
                                }
                            else: # No 'items' list, the dict itself might be a single node or contain other fields
                                gemini_result_for_return = parsed_data 
                                if isinstance(parsed_data, dict): 
                                     if "page_number" not in parsed_data: parsed_data["page_number"] = start_page_actual # Ensure single node gets page number
                                     processed_items_list.append(parsed_data)
                            parsed_data_for_file_saving = parsed_data
                            print(f"✅ Parsed Gemini dict response for single page {start_page_actual}.")
                        else: # Multi-page chunk but received a single, non-page-structured dict. THIS IS THE WARNING CASE.
                            error_msg = "Gemini returned a single dictionary for a multi-page chunk without recognized page structure (e.g. PageN keys or numeric 0,1 keys)."
                            print(f"⚠️ {error_msg} Chunk: {start_page_actual}-{start_page_actual + num_pages_in_chunk - 1}.")
                            
                            temp_parsed_data_copy = parsed_data.copy() # Work on a copy for file saving
                            if "page_number" not in temp_parsed_data_copy:
                                temp_parsed_data_copy["page_number"] = start_page_actual
                            temp_parsed_data_copy["error_ambiguous_multi_page_dict"] = error_msg
                            
                            items_in_parsed_data = temp_parsed_data_copy.get("items")
                            if isinstance(items_in_parsed_data, list):
                                 for node in items_in_parsed_data:
                                    if isinstance(node, dict):
                                        node["page_number"] = start_page_actual 
                                        node["page_number_assignment_warning"] = "Part of ambiguous multi-page dict. Defaulted."
                                 processed_items_list.extend(items_in_parsed_data)
                            elif isinstance(temp_parsed_data_copy, dict) and not items_in_parsed_data : # if it's just one node.
                                processed_items_list.append(temp_parsed_data_copy)

                            parsed_data_for_file_saving = temp_parsed_data_copy

                            gemini_result_for_return = {
                                "items": processed_items_list if processed_items_list else [temp_parsed_data_copy],
                                "error_details": error_msg,
                                "processed_chunk_page_range": [start_page_actual, start_page_actual + num_pages_in_chunk - 1],
                                "_source_data_type": "ambiguous_single_dict_multi_page"
                            }
                else: 
                    error_msg = f"Gemini response parsed to an unexpected data type: {type(parsed_data).__name__}"
                    # (rest of this error handling is the same)
                    print(f"⚠️ {error_msg} for chunk pages {start_page_actual}-{start_page_actual + num_pages_in_chunk - 1}.")
                    gemini_result_for_return = {
                        "error": error_msg,
                        "parsed_data_type": type(parsed_data).__name__,
                        "processed_chunk_page_range": [start_page_actual, start_page_actual + num_pages_in_chunk - 1],
                        "raw_output_preview": gemini_raw_text[:200]
                    }
                    parsed_data_for_file_saving = gemini_result_for_return
            
            except json.JSONDecodeError as je:
                # (This error handling is largely the same)
                print(f"⚠️ JSON DECODING ERROR on cleaned string (Chunk: {start_page_actual}-{start_page_actual + num_pages_in_chunk - 1}): {je}")
                error_msg = f"JSONDecodeError on cleaned string: {je}"
                gemini_result_for_return = {
                    "error": error_msg,
                    "cleaned_string_preview": cleaned_gemini_json_str[:200] if cleaned_gemini_json_str else "None",
                    "raw_output_preview": gemini_raw_text[:200],
                    "processed_chunk_page_range": [start_page_actual, start_page_actual + num_pages_in_chunk - 1]
                }
                parsed_data_for_file_saving = cleaned_gemini_json_str if cleaned_gemini_json_str else gemini_result_for_return
        else: 
            # (This error handling is largely the same)
            error_msg = "Could not extract/clean valid JSON string from Gemini response."
            if not gemini_raw_text: error_msg = "Empty raw response from Gemini."
            print(f"⚠️ {error_msg} for chunk pages {start_page_actual}-{start_page_actual + num_pages_in_chunk - 1}.")
            gemini_result_for_return = {
                "error": error_msg,
                "raw_output_preview": gemini_raw_text[:200],
                "processed_chunk_page_range": [start_page_actual, start_page_actual + num_pages_in_chunk - 1]
            }
            parsed_data_for_file_saving = gemini_result_for_return

        try:
            with open(output_file_path, "w", encoding="utf-8") as f:
                if isinstance(parsed_data_for_file_saving, (dict, list)):
                    json.dump(parsed_data_for_file_saving, f, indent=2, ensure_ascii=False)
                elif isinstance(parsed_data_for_file_saving, str): 
                    f.write(parsed_data_for_file_saving)
                else: 
                    json.dump({"error": "No valid structured data to save after cleaning/parsing attempts.",
                                "raw_output_preview": gemini_raw_text[:200]}, f, indent=2, ensure_ascii=False)
        except Exception as e_save:
            print(f"⚠️ Error saving processed Gemini data to file {output_file_path}: {e_save}")
            if isinstance(gemini_result_for_return, dict) and "error" not in gemini_result_for_return:
                 gemini_result_for_return["error_saving_file"] = str(e_save)
    
    # (Outer error handling remains largely the same, ensure page range uses num_pages_in_chunk)
    except json.JSONDecodeError as je: 
        if metrics.get("time_sec_gemini_layout", 0) == 0: metrics["time_sec_gemini_layout"] = time.time() - gemini_layout_start_time
        print(f"⚠️ Outer JSON DECODING ERROR (Chunk: {start_page_actual}-{start_page_actual + num_pages_in_chunk - 1}): {je}")
        metrics.update({"gemini_api_status": 500, "gemini_error_message": f"Outer JSONDecodeError: {je}"})
        gemini_result_for_return = {"error": f"Outer JSONDecodeError: {je}", "raw_output": gemini_raw_text, 
                                    "processed_chunk_page_range": [start_page_actual, start_page_actual + num_pages_in_chunk - 1]}
    except ValueError as ve: 
        if metrics.get("time_sec_gemini_layout", 0) == 0: metrics["time_sec_gemini_layout"] = time.time() - gemini_layout_start_time
        print(f"⚠️ Value Error (Gemini Call construction, Chunk: {start_page_actual}-{start_page_actual + num_pages_in_chunk - 1}): {ve}")
        metrics.update({"gemini_api_status": 400, "gemini_error_message": str(ve)}) 
        gemini_result_for_return = {"error": f"Gemini prompt/construction error: {ve}", 
                                    "processed_chunk_page_range": [start_page_actual, start_page_actual + num_pages_in_chunk - 1]}
    except Exception as e: 
        if metrics.get("time_sec_gemini_layout", 0) == 0: metrics["time_sec_gemini_layout"] = time.time() - gemini_layout_start_time
        print(f"⚠️ Gemini API error (Chunk: {start_page_actual}-{start_page_actual + num_pages_in_chunk - 1}): {e.__class__.__name__} - {e}")
        metrics.update({"gemini_api_status": 500, "gemini_error_message": f"{e.__class__.__name__}: {e}"})
        gemini_result_for_return = {"error": f"Gemini API call failed: {e}", 
                                    "processed_chunk_page_range": [start_page_actual, start_page_actual + num_pages_in_chunk - 1]}

    if "items" not in gemini_result_for_return and processed_items_list:
        gemini_result_for_return["items"] = processed_items_list
    elif "items" not in gemini_result_for_return and not processed_items_list and isinstance(gemini_result_for_return, dict) and "error" in gemini_result_for_return:
        gemini_result_for_return["items"] = [] 

    if isinstance(gemini_result_for_return, dict) and "processed_chunk_page_range" not in gemini_result_for_return:
        gemini_result_for_return["processed_chunk_page_range"] = [start_page_actual, start_page_actual + num_pages_in_chunk - 1]
        
    return gemini_result_for_return